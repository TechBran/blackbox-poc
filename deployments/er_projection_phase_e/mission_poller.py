...
class PollerCallbacks:
    ...
    `on_mission_arrival` is an OPTIONAL fire-and-forget signal invoked
    exactly once when the mission status transitions into "completed". It
    takes no args and returns nothing. Phase F wires this to the supervisor
    session so Gemini Live can narrate arrival with a fresh camera frame.
    Default is None to preserve backwards compatibility with existing
    call sites that construct PollerCallbacks with only the three required
    callbacks.
    """
    ...
    on_mission_arrival: Optional[Callable[[], Awaitable[None]]] = None
...
class MissionPoller:
    def __init__(
        ...
        *,
        ...
        self._arrival_fired: bool = False
    ...
    async def run(self) -> None:
        try:
            while not self._cancelled.is_set():
                ...
                # Fire arrival callback exactly once on transition into
                # completed, BEFORE send_terminal so any narration prep
                # (e.g. fresh frame fetch) starts queueing before the
                # supervisor's poller-shutdown logic runs. Errors are
                # swallowed because Phase F wires this to Gemini Live,
                # which can fail on transient network issues; we never
                # want such failures to leak into the mission terminal flow.
                if status == "completed" and not self._arrival_fired:
                    self._arrival_fired = True
                    if self._cb.on_mission_arrival is not None:
                        try:
                            await self._cb.on_mission_arrival()
                        except Exception as e:
                            try:
                                # The poller has no logger, fall back to print.
                                print(f"[mission_poller] arrival callback raised: {type(e).__name__}: {e}")
                            except Exception:
                                pass
