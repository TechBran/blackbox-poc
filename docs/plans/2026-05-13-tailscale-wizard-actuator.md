...
| E13 | Brandon's MSO2 Ultra testing pair_phone step 2026-05-17 | Phone paired successfully (Android app works, can chat) but wizard's pair_phone step doesn't detect it — shows QR/waiting view indefinitely. Two root causes intertwined: (a) no persistent paired-device registry (TOKEN_STORE is in-memory only; service restart wipes it); (b) /pair/claim never gets re-called once phone has stored credentials, so wizard polling against a new session's token never sees claimed=true. Resolves the long-standing TODO at onboarding_routes.py:185 (`replace with pairing_routes.list_claims()`). | Three coordinated changes: (a) pairing_routes.py adds persistent JSON registry at Manifest/paired_devices.json — /pair/claim now appends/updates on every claim. record_pairing() dedupes on device_name+device_kind. (b) /current-config wires its paired_devices field to pairing_routes.list_paired_devices() (resolves the TODO). (c) pair_phone.js on mount checks /current-config — if any paired_devices exist, shows 'Paired with X' success view directly + offers 'Pair another device' link for the existing mint-token flow. Wizard now correctly reflects multi-session pairing state. |
